...
|---|---|---|
| [setup-local-llm](https://github.com/sostrowsk/setup-local-llm) | **Models** | The model fleet: one `llama-server` per model, OpenAI-compatible, fixed ports — Qwen 3.6 35B-A3B chat (:8084), Qwen3-Embedding-8B (:8085), verified Qwen3-Reranker-8B (:8086), Gemma 4 12B/26B-MoE/31B (:8087–:8089), Whisper STT (:8090 Debian / mlx-whisper macOS), Chatterbox TTS multilingual (:8091) |
...
```
  ...
  Hermes Agent ────────►│  :8090 whisper-server (Debian, STT)           │
  (setup-hermes-honcho) │  :8091 Chatterbox TTS (multilingual, DE)      │
        │               └───────────────────────────────────────────────┘
        │ memory                    ▲
...
- **Open WebUI → fleet:** chat backends `:8084` (+ Gemma ports when served), RAG embeddings `:8085`, external reranker `:8086/v1/rerank` (full URL required), voice output via Chatterbox `:8091/v1` (`AUDIO_TTS_ENGINE=openai`).
...
| 8090 | whisper-server (Debian, STT) | setup-local-llm |
| 8091 | Chatterbox TTS (multilingual) | setup-local-llm |
